node_name           = "bewitched"
bao_addr            = "https://bao.local.ni-coni-coni.com"
snippets_datastore  = "ci-snippets"
ssh-key             = " coni@lin"

vms = {
  "main" = {
    vm_id     = 500
    cores     = 2
    memory    = 4096
    disk_size = 20
    host_path = "/bewitched-raidz2/server/main/"
    networks = [
      { bridge = "vmbr0" },
      { bridge = "main" },
    ]
    ips = [
      { address = "192.168.1.197/24", gateway = "192.168.1.254" },
      { address = "10.0.3.1/24" },
    ],
    services  = ["authentik", "pterodactyl_panel"]
  },
  "nextcloud" = {
    vm_id     = 501
    cores     = 2
    memory    = 4096
    disk_size = 10
    host_path = "/bewitched-raidz2/server/nextcloud/"
    networks = [
      { bridge = "vmbr0" },
      { bridge = "nextcloud" },
    ]
    ips = [
      { address = "192.168.1.196/24", gateway = "192.168.1.254" },
      { address = "10.0.2.1/24" },
    ],
    services  = ["nextcloud"]
  },
  "wings" = {
    vm_id     = 502
    cores     = 4
    memory    = 16384
    disk_size = 100
    host_path = "/bewitched-raidz2/server/wings/"
    networks = [
      { bridge = "vmbr0" },
      { bridge = "main" }
    ]
    ips = [
      { address = "192.168.1.192/24", gateway = "192.168.1.254" },
      { address = "10.0.3.3/24" },
    ],
    services  = []
  }
}


